...
public class ConversationDAO {
    ...
    public int createDirectConversation(String tipo) {
        ...
        try (Connection conn = DatabaseConnection.connect(); // conexion
    ...
    */
    ...
    try (Connection conn = DatabaseConnection.connect()) {
        conn.setAutoCommit(false); // Todo o nada — si algo falla, se revierte todo

        try (
            PreparedStatement s1 = conn.prepareStatement(deleteMsgs);
            PreparedStatement s2 = conn.prepareStatement(deleteMembers);
            PreparedStatement s3 = conn.prepareStatement(deleteConv)
        ) {
            ...
            conn.commit(); // Confirmar los tres pasos juntos
